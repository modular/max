...
def test_split():
    ...
    assert_equal(StaticString("1,,,3").split(","), L("1", "", "", "3"))
    assert_equal(StaticString(",,,").split(","), L("", "", "", ""))
    assert_equal(StaticString(" a b ").split(" "), L("", "a", "b", ""))
    assert_equal(StaticString("abababaaba").split("aba"), L("", "b", "", ""))
    assert_true(len(StaticString("").split()) == 0)
    assert_true(len(StaticString(" ").split()) == 0)
    assert_true(len(StaticString("").split(" ")) == 1)
    assert_true(len(StaticString(",").split(",")) == 2)
    assert_true(len(StaticString(" ").split(" ")) == 2)
    # assert_true(len("".split("")) == 2) # TODO(#3528)
    assert_true(len(StaticString("  ").split(" ")) == 3)
    assert_true(len(StaticString("   ").split(" ")) == 4)

    # should split into maxsplit + 1 items
    assert_equal(StaticString("1,2,3").split(",", 0), L("1,2,3"))
    assert_equal(StaticString("1,2,3").split(",", 1), L("1", "2,3"))
    ...
    # Split in middle
    assert_equal(StaticString("faang").split("n"), L("faa", "g"))

    # No match from the delimiter
    assert_equal(StaticString("hello world").split("x"), L("hello world"))

    # Multiple character delimiter
    assert_equal(StaticString("hello").split("ll"), L("he", "o"))

    res = L("", "bb", "", "", "", "bbb", "")
    assert_equal(StaticString("abbaaaabbba").split("a"), res)
    assert_equal(StaticString("abbaaaabbba").split("a", 8), res)
    s1 = StaticString("abbaaaabbba").split("a", 5)
    assert_equal(s1, L("", "bb", "", "", "", "bbba"))
    assert_equal(StaticString("aaa").split("a", 0), L("aaa"))
    assert_equal(StaticString("a").split("a"), L("", ""))
    assert_equal(StaticString("1,2,3").split("3", 0), L("1,2,3"))
    assert_equal(StaticString("1,2,3").split("3", 1), L("1,2,", ""))
    assert_equal(StaticString("1,2,3,3").split("3", 2), L("1,2,", ",", ""))
    assert_equal(StaticString("1,2,3,3,3").split("3", 2), L("1,2,", ",", ",3"))

    assert_equal(StaticString("Hello 🔥!").split(), L("Hello", "🔥!"))

    s2 = StaticString("Лорем ипсум долор сит амет").split(" ")
    assert_equal(s2, L("Лорем", "ипсум", "долор", "сит", "амет"))
    s3 = StaticString("Лорем ипсум долор сит амет").split("м")
    assert_equal(s3, L("Лоре", " ипсу", " долор сит а", "ет"))

    # TODO(#3528)
    # assert_equal(StaticString("123").split(""), L("", "1", "2", "3", ""))
    # assert_equal(StaticString("").join(StaticString("123").split("")), "123")
    # assert_equal(StaticString(",1,2,3,").split(","), StaticString("123").split(""))
    # assert_equal(StaticString(",").join(StaticString("123").split("")), ",1,2,3,")
